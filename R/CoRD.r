...
CoRD=function(Datos_Insumo,Req_Int,Cantidad_selec,Filtrar_Alimentos=NULL,EER=NULL){
  ...
  if ("Sexo" %in% colnames(Req_Int)) {
    Sexos <- split(Req_Int, Req_Int$Sexo);sexo_nombre=names(Sexos)
    
    if(!is.null(EER)){EER_p <- split(EER, EER$Sexo)}
  ...
  for (sexo_nombre in sexo_nombre) { 
    ...
    if(!is.null(EER)){EER_S <- EER_p[[sexo_nombre]]}
    ...
    for (E in Edad) {
      ...
      if(!is.null(EER)){
        Costo_1000kcal=(costo_edad/EER_S$Energia[EER_S$Edad==E])*1000
        
        assign("EER_S",EER_S,envir = globalenv())
        assign("E",E,envir = globalenv())
        assign("costo_edad",costo_edad,envir = globalenv())
        
        
      df_temp <- data.frame(Edad = E, Costo = costo_edad,Costo_1000kcal=Costo_1000kcal)
      }else{
